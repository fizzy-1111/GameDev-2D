        ...
        {          
            if (node.Active)
            {
                ...
                if (node.Item.TimeToLive > 0)
                {
                    ...
                    int result = -1;
                    RaycastHit2D isTouching=new RaycastHit2D();
                    if (CollisionDetection == CollisionDetectionType.Raycast)
                    {
                        ...
                        isTouching = Physics2D.Raycast(node.Item.Position, deltaPosition);
                    }
                    ...
                    if (result > 0)
                    {
                        ...
                        if (isTouching.collider.tag == "Player")
                        {
                            GameManager.Instance.player.hitPoint -= 1;
                            ReturnNode(node);
                        }
                        if (isTouching.collider.name =="Head1")
                        {
                            ReturnNode(node);
                        }
